...
import { Dashboard, Clients, Demands, Payments, Documents, Services, Calendar, Tasks, Pipeline } from './components';
import { useAlertModal, AlertModalComponent } from './utils/alertModal';
import { useAuth } from '../../../contexts/AuthContext';
import { useMClientsData } from '../../../hooks/useMClientsData';
import { deleteClientApi } from '../../../services/mclientsApi';

const MClients = () => {
  const { user } = useAuth();
  
  // Sistema de username local (por enquanto)
  const getUsername = () => {
    if (user?.username) return user.username;
    // Se não tiver username, gerar ou usar do localStorage
    let username = localStorage.getItem('mclients_username');
    if (!username) {
      // Gerar username baseado no email ou nome
      const base = user?.email?.split('@')[0] || user?.name?.toLowerCase().replace(/\s+/g, '') || 'user';
      username = base + Math.floor(Math.random() * 1000);
      localStorage.setItem('mclients_username', username);
    }
    return username;
  };
  ...
  // Usar hook customizado para gerenciar dados com cache e sincronização otimizada
  const { data: mclientsData, setData: updateData, setDataSilent, refreshData, loading: dataLoading, saving } = useMClientsData();
  ...
  const {
    ...
    tasks: dataTasks = [],
    timeEntries: dataTimeEntries = [],
    activities: dataActivities = []
  } = mclientsData;
  
  // Função helper para atualizar dados (com sincronização automática)
  // Permite forçar salvamento imediato quando necessário
  const updateAllData = (updates, immediate = false) => {
    const newData = {
      ...mclientsData,
      ...updates
    };
  ...
  // Função helper para atualizar dados com salvamento imediato
  const updateAllDataImmediate = (updates) => {
    updateAllData(updates, true);
  };

  // Função helper para atualizar dados SEM salvar (útil após DELETE)
  const updateAllDataSilent = (updates) => {
    const newData = {
      ...mclientsData,
      ...updates
    };
  ...
  // Setters individuais que atualizam o estado e sincronizam automaticamente
  const setClients = (newClients, immediate = false) => updateAllData({ clients: newClients }, immediate);
  const setDemands = (newDemands, immediate = false) => updateAllData({ demands: newDemands }, immediate);
  const setPayments = (newPayments) => updateAllData({ payments: newPayments });
  const setDocuments = (newDocuments) => updateAllData({ documents: newDocuments });
  
  // Setters silenciosos (não salvam no backend - útil após DELETE)
  const setClientsSilent = (newClients) => updateAllDataSilent({ clients: newClients });
  const setDemandsSilent = (newDemands) => updateAllDataSilent({ demands: newDemands });
  const setPaymentsSilent = (newPayments) => updateAllDataSilent({ payments: newPayments });
  const setDocumentsSilent = (newDocuments) => updateAllDataSilent({ documents: newDocuments });
  const setServices = (newServices) => updateAllData({ services: newServices });
  const setTasks = (newTasks) => updateAllData({ tasks: newTasks });
  const setTimeEntries = (newEntries) => updateAllData({ timeEntries: newEntries });
  const setActivities = (newActivities) => updateAllData({ activities: newActivities });
  
  // Usar dados do hook ou valores padrão
  const clients = dataClients;
  const demands = dataDemands;
  const payments = dataPayments;
  const documents = dataDocuments;
  const services = dataServices;
  const tasks = dataTasks;
  const timeEntries = dataTimeEntries;
  const activities = dataActivities;
  
  const [activeTab, setActiveTab] = useState('dashboard'); // dashboard, clients, demands, payments, documents, services, calendar, tasks, pipeline
  ...
  // Modal de alerta/confirmação customizado usando hook
  const { alertModal, setAlertModal, showAlert, showConfirm } = useAlertModal();

  // Estados principais - agora gerenciados pelo hook useMClientsData
  // Os dados são carregados do backend e mantidos em cache local
  // Estados locais que não precisam ser sincronizados com backend
  ...
  // activities agora vem do hook useMClientsData
  ...
  // timeEntries agora vem do hook useMClientsData
  ...
  const handleAddClient = () => {
    ...
    // Salvar imediatamente para garantir persistência no backend
    setClients([...clients, newClient], true);
    
    // Dados serão salvos automaticamente pelo hook useMClientsData
  ...
  const handleDeleteClient = async (id) => {
    showConfirm(
      ...
      async () => {
        try {
          // Resolver ID real: usar email e o estado mais recente do backend
          const clientLocal = clients.find(c => c.id === id);
          const email = clientLocal?.email;
          const clientFromBackend = (mclientsData?.clients || []).find(c =>
            (email && c.email === email) || c.id === id
          );
          const resolvedId = clientFromBackend?.id ?? id;

          // Deletar do banco de dados
          await deleteClientApi(resolvedId);
          
          // Atualizar estado local sem re-salvar no backend
          const deleteIdStr = resolvedId?.toString();
          const deleteIdNum = typeof resolvedId === 'number' ? resolvedId : parseInt(resolvedId);
          
          const filteredClients = clients.filter(c => {
            const clientIdStr = c.id?.toString();
            const clientIdNum = typeof c.id === 'number' ? c.id : parseInt(c.id);
            return clientIdStr !== deleteIdStr &&
                   clientIdNum !== deleteIdNum &&
                   c.id !== resolvedId;
          });

          setClientsSilent(filteredClients);
          setDemandsSilent(demands.filter(d => {
            const demandClientId = d.clientId?.toString();
            return demandClientId !== deleteIdStr && d.clientId !== resolvedId;
          }));
          setPaymentsSilent(payments.filter(p => {
            const paymentClientId = p.clientId?.toString();
            return paymentClientId !== deleteIdStr && p.clientId !== resolvedId;
          }));
          setDocumentsSilent(documents.filter(d => {
            const docClientId = d.clientId?.toString();
            return docClientId !== deleteIdStr && d.clientId !== resolvedId;
          }));

          // Recarregar dados para garantir sincronização com backend
          if (refreshData) {
            await refreshData();
          }
          ...
          console.error('[DELETE CLIENT] Erro ao excluir cliente:', error);
  ...
  // Dados agora são gerenciados pelo hook useMClientsData


  // Dados são salvos automaticamente pelo hook useMClientsData
  ...
  return (
    <>
      ...
      {/* Header */}
      <div className="bg-white border-gray-200 border-b px-4 md:px-8 py-4 md:py-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-semibold text-gray-900 mb-1 font-poppins">MClients</h1>
            <p className="text-xs md:text-sm text-gray-500 font-poppins">Gestão completa de clientes e projetos</p>
          </div>
          <div className="flex items-center gap-4 md:gap-6">
            <div className="flex-1 md:flex-none text-center">
              <div className="text-xs text-gray-500 font-poppins mb-1">Clientes</div>
              <div className="text-lg md:text-xl font-semibold text-gray-900 font-poppins">{clients.length}</div>
            </div>
            <div className="flex-1 md:flex-none text-center">
              <div className="text-xs text-gray-500 font-poppins mb-1">Demandas</div>
              <div className="text-lg md:text-xl font-semibold text-gray-900 font-poppins">
      ...
      <div className="bg-white border-gray-200 border-b">
        {/* Desktop: Grid layout */}
        <div className="hidden md:flex flex-wrap gap-1 px-8 py-2">
          {[
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-3 border-b-2 font-medium text-sm transition-colors font-poppins whitespace-nowrap ${
                  activeTab === tab.id
            ...
            {[
              ...
              return (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-1.5 px-3 py-2.5 border-b-2 font-medium text-sm transition-colors font-poppins whitespace-nowrap rounded-t-md ${
                    activeTab === tab.id
                      ? 'border-gray-900 text-gray-900 bg-gray-50'
                      : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-25'
                  }`}
                >
                  <Icon className="w-4 h-4" />
                  {tab.name}
      ...
      <div className="flex-1 overflow-auto px-4 md:px-8 py-6 md:py-8">
        ...
        {activeTab === 'clients' && (
          <Clients
            ...
            getClientDemands={getClientDemands}
